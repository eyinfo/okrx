...
import io.github.eyinfo.okrx.annotations.ApiHeadersCall;
import io.github.eyinfo.okrx.annotations.BYTES;
import io.github.eyinfo.okrx.annotations.BaseUrlTypeName;
import io.github.eyinfo.okrx.annotations.DELETE;
import io.github.eyinfo.okrx.annotations.DataCallStatus;
import io.github.eyinfo.okrx.annotations.DataParam;
import io.github.eyinfo.okrx.annotations.DelQuery;
import io.github.eyinfo.okrx.annotations.GET;
import io.github.eyinfo.okrx.annotations.Header;
import io.github.eyinfo.okrx.annotations.HeaderPart;
import io.github.eyinfo.okrx.annotations.Headers;
import io.github.eyinfo.okrx.annotations.PATCH;
import io.github.eyinfo.okrx.annotations.POST;
import io.github.eyinfo.okrx.annotations.PUT;
import io.github.eyinfo.okrx.annotations.Param;
import io.github.eyinfo.okrx.annotations.ParamList;
import io.github.eyinfo.okrx.annotations.Path;
import io.github.eyinfo.okrx.annotations.RequestTimeLimit;
import io.github.eyinfo.okrx.annotations.RequestTimePart;
import io.github.eyinfo.okrx.annotations.RetCodes;
import io.github.eyinfo.okrx.annotations.UrlItem;
import io.github.eyinfo.okrx.annotations.UrlItemKey;
...
import io.github.eyinfo.okrx.enums.RuleParams;
    ...
    private <T> boolean bindRequestTypes(Annotation[] declaredAnnotations, Method method, Class<T> apiClass, RetrofitParams retrofitParams, Object[] args) {
        ...
        for (Annotation declaredAnnotation : declaredAnnotations) {
            if (declaredAnnotation.annotationType() == POST.class) {
                ...
                bindHeadersAnnotation(method, retrofitParams, args, isRemoveEmptyValueField);
...
//        int notSpecifiedParamKeyCount = 0;//记录未指定参数key对应的参数个数
    ...
    private <T> void matchRequestUrl(Class<T> apiClass,
        ...
        if (methodRequestContentType != null && methodRequestContentType != RequestContentType.None) {
            retrofitParams.setRequestContentType(methodRequestContentType);
        } else {
        ...
        if (urlTypeName == null) {
            ...
            if (isFullUrl) {
                ...
                retrofitParams.setRequestUrl(url);
                retrofitParams.setIsJoinUrl(!ValidUtils.valid(RuleParams.Url.getValue(), url));
            }
        }
    }
    ...
    private void bindHeadersAnnotation(Method method, RetrofitParams retrofitParams, Object[] args, boolean isRemoveEmptyValueField) {
            ...
            if (!ObjectJudge.isNullOrEmpty(values)) {
                ...
                for (String value : values) {
                    ...
                    if (lst.length == 2 && !TextUtils.isEmpty(lst[0])
                        ...
                        String patient = String.format(RuleParams.MatchTagBetweenContent.getValue(), "\\{", "\\}");
                        String match = ValidUtils.matche(patient, lst[1]);
                        if (TextUtils.isEmpty(match)) {
                            ...
                            if (!ObjectJudge.isNullOrEmpty(paramAnnotationObject)) {
                                for (Map.Entry<Integer, HeaderPart> headerPartIntegerEntry : paramAnnotationObject.entrySet()) {
                                    HeaderPart headerPart = headerPartIntegerEntry.getValue();
                                    if (TextUtils.equals(headerPart.value(), match)) {
                                        String dataValue = String.valueOf(args[headerPartIntegerEntry.getKey()]);
                                        if (annotation.isRemoveEmptyValueField()) {
                                            if (!TextUtils.isEmpty(dataValue)) {
