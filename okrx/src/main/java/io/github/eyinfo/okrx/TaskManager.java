...
import io.github.eyinfo.okrx.beans.TaskEntry;
...
public class TaskManager {
    ...
    private SoftReference<HashMap<String, TaskEntry<? extends Runnable>>> reference = new SoftReference<>(map);
    ...
    private class TaskRunnable<T extends Runnable> extends RunnableParamsN {
        ...
        public void run(Object... params) {
            ...
            //执行计数
            int performCounts = ConvertUtils.toInt(params[1]);
            //延迟执行时间
            long delayTime = ConvertUtils.toLong(params[2]);
            //执行方式
            String implementWay = String.valueOf(params[4]);
            //任务是否自动执行
            boolean isAutoNextTask = ObjectJudge.isTrue(params[5]);
            HashMap<String, TaskEntry<? extends Runnable>> taskMap = getTaskMap();
            //任务对象
            T task = (T) params[3];
            task.run();
    ...
    private void execute(TaskEntry<? extends Runnable> taskEntry, String implementWay, boolean isAutoNextTask) {
        ...
        if (taskEntry.getDelayTime() > 0) {
            HandlerManager.getInstance().postDelayed(taskRunnable, taskEntry.getDelayTime(), taskEntry.getKey(), taskEntry.getPerformCounts(), taskEntry.getDelayTime(), task, implementWay, isAutoNextTask);
        } else {
            taskRunnable.run(taskEntry.getKey(), taskEntry.getPerformCounts(), taskEntry.getDelayTime(), task, implementWay, isAutoNextTask);
        }
    }
